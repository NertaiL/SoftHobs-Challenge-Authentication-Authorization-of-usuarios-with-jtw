...
import { byEmail } from "../models/registerModel.js"; //importamos email que tenemos en el modelo del usuario.

export const loginUser = async (req, res) => {
  // si pasa la validacion del middlewares que creamos pasa al siguiente codigo
  const { email,password } = req.body;  //AQUI QUEDARIA LOGEADO LO QUE ESCRIBA EL USUARIO EN LA PAGINA QUE EN ESTE CASO SERIA EMAIL Y PASSWORD
  try {
    const findUser = await byEmail(email); //aqui me retorna el correo que busque en la base de datos
    if (!findUser) {
      ...
      const isPasswordValid = bcrypt.compareSync(
        ...
        password, //esta vendria siendo la password que ingreso el cliente para iniciar sesion
      ...
      if (!isPasswordValid) {
        ...
        const { email, rol } = findUser; //aqui podriamos  pasar solo email los otros 2 no lo ocupamos
        const token = jwt.sign({ email }, process.env.JWT_SECRET, {
          //aqui le paso el process.env.JWT_SECRET que es el que tenemos en nuestro env
          expiresIn: "6h", //expires in "1h" quiere decir que el token creado durara solamente 1 hora y despues no va a servir, osea el usuario va a tener que autenticar nuevamente
        });
        res.status(200).json({
          //bueno aqui retorno un mensaje, codigo 200, con el token ,lo importante es retornar el token los otros 2 da lo mismo
          message: `Bienvenido, tu rol es ${rol} y has iniciado sesion`,
          code: 200,
          token,
        });
      }
